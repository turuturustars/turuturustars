-- Refine announcement WhatsApp copy after the reliable queue path is live.
-- This keeps the already-applied migration immutable and applies the updated
-- member-facing message format under a new migration version.
...
begin
  ...
  v_whatsapp_message := left(
    'Turuturu Stars Announcement' || E'\n\n' ||
    v_title ||
    case
      when v_priority in ('urgent', 'high') then E'\nPriority: ' || upper(v_priority)
      else ''
    end ||
    E'\n\n' ||
    left(v_message, 1200) ||
    E'\n\nView this notice in your member portal:' || E'\n' ||
    'https://turuturustars.co.ke' || v_action_url,
    3900
  );
